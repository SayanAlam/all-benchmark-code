...
def gen(path, outpath, model_path):
    ...
    with open(outpath, 'a') as outf:
        for start in trange(0, len(data), batch_size):
            ...
            # qwen 3 style model prompt 
            # queries = [] 
            # for d in batch_data: 
            #     messages = [ {"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": d['prompt']} ]
...
def _delete_if_exists(filepath):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Evaluate base model on SafetyBench")
    parser.add_argument(
        "--model",
        type=str,
    ...
    if eva_set == 'en':
        # for English — delete previous output files
        outpath_prompts = f'../data/test_en_eva_{model_name}_zeroshot{zero_shot}_prompts.json'
        outpath_res = f'../data/test_en_eva_{model_name}_zeroshot{zero_shot}_res.jsonl'
        outpath_processed = f'../data/test_en_eva_{model_name}_zeroshot{zero_shot}_res_processed.json'
        _delete_if_exists(outpath_prompts)
        _delete_if_exists(outpath_res)
        _delete_if_exists(outpath_processed)
        # construct evaluation prompts
        path = '../opensource_data/test_en.json'
        outpath = outpath_prompts
        shotpath = '../data/dev_en.json'
        en = True
        construct_evaluate_prompts(path, outpath, en=en, zero_shot=zero_shot, shot_path=shotpath)
        # generate the responses
        path = outpath_prompts
        outpath = outpath_res
        gen(path, outpath, model_name)
        # extract answers from the responses
        path = outpath_res
        outpath = outpath_processed
        process_medium_results(path, outpath)

    elif eva_set == 'zh':
        # for Chinese — delete previous output files
        outpath_prompts = f'../data/test_zh_eva_{model_name}_zeroshot{zero_shot}_prompts.json'
        outpath_res = f'../data/test_zh_eva_{model_name}_zeroshot{zero_shot}_res.jsonl'
        outpath_processed = f'../data/test_zh_eva_{model_name}_zeroshot{zero_shot}_res_processed.json'
        _delete_if_exists(outpath_prompts)
        _delete_if_exists(outpath_res)
        _delete_if_exists(outpath_processed)
        ...
        # for Chinese subset — delete previous output files
        outpath_prompts = f'../data/test_zh_subset_eva_{model_name}_zeroshot{zero_shot}_prompts.json'
        outpath_res = f'../data/test_zh_subset_eva_{model_name}_zeroshot{zero_shot}_res.jsonl'
        outpath_processed = f'../data/test_zh_subset_eva_{model_name}_zeroshot{zero_shot}_res_processed.json'
        _delete_if_exists(outpath_prompts)
        _delete_if_exists(outpath_res)
        _delete_if_exists(outpath_processed)
